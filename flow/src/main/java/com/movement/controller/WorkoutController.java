...
public class WorkoutController extends BaseController {
	...
	/**
	 * Return all of the workouts created by a user
	 * @param id (corresponds to user.id)
	 * @param size
	 * @param page
	 * @return
	 * @throws NoPermissionException
	...
	@ResponseBody
	public ResponseList<Workout> getMyWorkouts(@PathVariable Long id, @Param(PARAM_SIZE) int size, 
			@Param(PARAM_PAGE) int page) throws NoPermissionException{
		User current = getLoggedInUser();
		checkUserPermission(current);
		Page<Workout> workouts = workoutService.findAllWorkoutsByUser(id, new PageRequest(page, size));
		return new ResponseList<Workout>(workouts);
	}
}
